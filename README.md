# 👋 Hi, I'm Pranjal Tripathi  

🎓 B.Tech IT Student (GCET) | 💻 Web Developer | 🧠 DSA & LeetCode Enthusiast  

I am an Information Technology undergraduate passionate about **problem solving, web development, and data-driven systems**.  
I enjoy building real-world projects, solving DSA problems on LeetCode, and continuously improving my development skills.

---

## 🧠 Coding & Problem Solving  

<p align="center">
  <a href="https://leetcode.com/u/Pranjal2003leet/">
    <img src="https://leetcard.jacoblin.cool/Pranjal2003leet?theme=dark&font=Karma&ext=heatmap&border=2&radius=16" width="600"/>
  </a>
</p>

- ✅ **185+ problems solved on LeetCode**
- ✅ **126+ problems solved on GeeksforGeeks**
- Strong grip on **DSA, arrays, strings, recursion, basic graphs**

---

## 🚀 Projects  

| Project | Description | Tech Stack | Live / Repo |
|-------|------------|-----------|-------------|
| **Tripathi Weather App** | Real-time weather forecast web app with city search and error handling. | HTML, CSS, JavaScript, OpenWeatherMap API | [Live](https://pranjaltripathi007.github.io/Pranjal-Weather-App/) |
| **Advanced To-Do Web App** | Task creation, editing, filters, dark mode & LocalStorage support. | HTML, CSS, JavaScript | [Live](https://pranjaltripathi777.github.io/To-do-app/) |

---

## 🛠️ Technical Skills  

### 👨‍💻 Programming
- C, C++
- Java (Basics)

### 🌐 Web Development
- HTML, CSS, JavaScript
- Flask
- REST APIs

### 🗄️ Database
- MySQL

### ⚙️ Tools & Platforms
- Git & GitHub
- GitHub Pages (Deployment)
- VS Code

### 🧠 CS Fundamentals
- Data Structures & Algorithms  
- Basic Linear Algebra, Probability & Statistics  

---

## 🎓 Education  

- **B.Tech in Information Technology**  
  Galgotias College of Engineering & Technology (2023 – 2027)  
  CGPA: **7.0 / 10** (till 4th semester)

- **Class XII** – 71.5%  
- **Class X** – 85%

---

## 📜 Certifications  

- AI Appreciate Badge – *AI For All*  
- AI Aware Badge – *AI For All*  
- GenAI Powered Data Analytics – *Forage*  
- Quantum Data Analytics Job Simulation – *Forage*  
- Neo4j Certified Professional – *Neo4j*

---

## 📫 Connect With Me  

- 💼 GitHub: [pranjaltripathi777](https://github.com/pranjaltripathi777)  
- 🧠 LeetCode: [Pranjal2003leet](https://leetcode.com/u/Pranjal2003leet/)  
- 💼 LinkedIn: [Pranjal Tripathi](https://www.linkedin.com/in/pranjal-tripathi-470624205/)  

---

⭐ *If you like my work, consider starring my repositories!*  

